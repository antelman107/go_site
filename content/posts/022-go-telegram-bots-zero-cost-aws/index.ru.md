...
Общение с ботом работает через Telegram webhook: когда пользователь что-то пишет, Telegram делает HTTP-запрос по Function URL. Lambda сразу отвечает `200 OK`, а настоящую работу продолжает на втором вызове с более длинным таймаутом. В рамках этих огромных лимитов я не плачу за выполнение функций ни цента. Для фоновых задач я использую **Amazon EventBridge**, который дергает лямбды по расписанию.
...
Сейчас чат идёт через **[Google ADK for Go v2](https://pkg.go.dev/google.golang.org/adk/v2)** (`LlmAgent` + `Runner`). Runner вызывает Gemini, сразу выполняет tools против Telegram и S3, возвращает результат функции (включая `message_id` нового опроса) и даёт модели вызвать следующие tools. Я ограничиваю это **6** раундами `GenerateContent`, чтобы застрявший цикл не сорвал **120-секундный** таймаут webhook-Lambda. Несколько tool-вызовов в одном ответе модели выполняются по порядку.
...
Важно учитывать HTTP-статус webhook: для Telegram успех — только `2xx`. Если функция вернула `4xx/5xx` или не ответила вовремя, Telegram будет повторять запрос много раз. Telegram ждёт HTTP-ответ около **60 секунд**. Ход ADK с несколькими раундами Gemini и вызовами Telegram Bot API легко выходит за этот лимит, хотя сама AWS Lambda может работать дольше.

Поэтому обработчик Function URL не обрабатывает апдейт на HTTP-запросе. Он кладёт тело Telegram в небольшой конверт (`type: telegram-update`) и **асинхронно вызывает ту же Lambda** (`InvocationTypeEvent`). Затем сразу возвращает Telegram `200 OK`. Второй вызов уже не ограничен ожиданием Telegram: у него таймаут **120 секунд**, там крутится цикл ADK, пишется S3 и ходят запросы в Bot API. Если self-invoke не удался, первый вызов обрабатывает апдейт сам, чтобы ничего не потерять.

Дополнительный invoke всё равно считается одним запросом Lambda — при миллионе бесплатных вызовов это дёшево. IAM-роль разрешает `lambda:InvokeFunction` только на ARN этой же функции.

Воркер после обработки тоже отвечает `200` (и на невалидный JSON от Telegram тоже), чтобы редкий inline-fallback не запускал ретраи Telegram.
