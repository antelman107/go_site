...
Я использую промпт на несколько типов запросов:
1. **Создание опроса на игру** (дата, время, место, лимиты, цена и т.д.), включая пакетный вариант «несколько игр сразу».
...
4. **Обновление памяти** (запомнить или забыть устоявшийся факт группы).
5. **Без ответа** (реакции и прочий шум, на который боту не нужно отвечать).

Для каждого типа в системном промпте задан свой формат JSON. ИИ должен вернуть только один JSON-вариант в зависимости от ситуации. Дальше код парсит этот JSON в соответствующую Go-структуру и по типу структуры понимает следующий шаг: создать игру, изменить ее, сохранить память, отправить обычный текстовый ответ или промолчать.

### Контекст и модель

У ИИ есть контекст из последних 50 сообщений каждого чата, связанных с ботом (упоминания его ника или ответы на его сообщения). История хранится в S3 как JSON-массив сообщений, который затем декодируется в структуру для Gemini. Это *рабочее окно* диалога, а не база знаний: факты в нем протухают, и нельзя точечно обновить или забыть одну норму группы, не переписывая транскрипт.
...
Память меняется двумя способами:

1. **Явная просьба запомнить или забыть.** Если кто-то пишет «запомни», «forget» или «теперь зал вот этот», Gemini возвращает JSON с `"type": "memory update"`, коротким подтверждением и списком `memory_ops`.
2. **Тихий вывод.** Если модель замечает устойчивую норму группы при создании игры или даже когда ответ не нужен, она прикрепляет `memory_ops` к основному JSON (`new game`, `modified game`, `no reply`) и не шлет отдельное «я запомнил». Разовые голоса и разовые платежи в память не попадают.

Операции только `upsert` и `delete`. Go-обработчик нормализует ключи, применяет операции к списку фактов этого чата и один раз сохраняет стейт в конце Lambda.

```json
{
  "type": "memory update",
  "message": "Запомнил пляжную площадку.",
  "memory_ops": [
    {"op": "upsert", "key": "beach_location", "text": "Пляжный волейбол: Kilyos Beach"},
